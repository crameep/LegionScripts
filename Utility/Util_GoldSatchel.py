...
from LegionUtils import (
    ...
    ErrorManager, CooldownTracker, ResourceRateTracker,
    get_item_count,
    # Phase 4: Complex systems
    HotkeyManager, WindowPositionTracker
)
...
COLLAPSED_HEIGHT = 24
NORMAL_HEIGHT = 118
CONFIG_HEIGHT = 266  # Normal height + config panel (148px - config buttons)
...
# Income tracking
income_tracker = ResourceRateTracker("gold", check_interval=2.0, notify_threshold=100)
...
def update_display():
    ...
    try:
        ...
        per_min, per_10min, per_hour = income_tracker.get_rates()
        ...
        if income_display_mode == 0:  # Compact - rates only
            ...
            income_text = format_gold_compact(income_tracker.total_gained) + " (" + format_gold_compact(per_min) + "/m)"
...
configBg.SetRect(0, config_y, WINDOW_WIDTH_CONFIG, 148)  # Config panel height
configBg.IsVisible = False
gump.Add(configBg)
...
while not API.StopRequested and not script_should_stop:
    try:
        ...
        income_tracker.update(count_all_gold())
